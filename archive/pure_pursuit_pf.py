...
class PurePursuitController(Node):
    def __init__(self):
        super().__init__('pure_pursuit_pf')
        ...
        self.min_speed = 1.0  # minimum speed
        self.max_speed = 3.3  # maximum speed
        self.min_dist_point_reached = 0.3  # minimum value for dist_point_reached
        self.max_dist_point_reached = 1.0  # minimum value for dist_point_reached
    ...
    def calculate_control_commands(self):
        ...
        if self.current_pose and self.waypoints.any():
            ...
            speed = self.min_speed + (1 - 2* (abs(ackermann_cmd.drive.steering_angle) / self. max_steering_angle) **2) * (self.max_speed - self.min_speed)
            ackermann_cmd.drive.speed = np.clip(speed, self.min_speed, self.max_speed)
    ...
    def calculate_point_reached_dist(self, steering_angle):
        self.dist_point_reached = self.min_dist_point_reached + (1 - (abs(steering_angle) / self.max_steering_angle)) * (self.max_dist_point_reached - self.min_dist_point_reached)
    ...
    def read_waypoints_from_csv(self, waypoints_file):
        self.waypoints = np.genfromtxt(waypoints_file, delimiter=";", comments='#')[:, 1:3]
