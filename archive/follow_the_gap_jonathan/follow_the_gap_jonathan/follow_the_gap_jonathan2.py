...
class FollowTheGapNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('error_based_velocities.low', 1.0)
        self.declare_parameter('error_based_velocities.medium', 0.7)
        self.declare_parameter('error_based_velocities.high', 0.5)
        self.declare_parameter('half_vehicle_width', 0.3)
        self.declare_parameter('max_steering_angle', 1.57)
    ...
    def process_laserscan(self, scan_msg):
        ...
        scan_msg.ranges = [x for x in (np.clip(scan_msg.ranges, 0.0, 7.0)).tolist()]
        ...
        return start_index, end_index, closest_dist, closest_angle, closest_idx
    ...
    def follow_the_gap_algorithm(self, scan_msg):
        ...
        start_index, end_index, closest_dist, closest_angle, closest_idx = self.process_laserscan(scan_msg)
        middle_index = len(scan_msg.ranges) // 2

        # Step 2 : Zero out the elements within the 'bubble_radius' around the 'center_index'
        ...
        # Step 4: Find angle(s) with largest range value within max gap
            ...
            steering_angle = np.clip(steering_angle, -self.get_parameter('max_steering_angle').get_parameter_value().double_value, \
                                     self.get_parameter('max_steering_angle').get_parameter_value().double_value )

        # Step 5: check if desired steering angle leads to collision
            # checking for clearance on the right when steering to the right
            if abs(steering_angle) < 0.20:
                if steering_angle > 0:
                    for i, range_val in enumerate(reversed(scan_msg.ranges[middle_index:(end_index + 1)])):
                        if range_val > 0:
                            index_i = end_index - i
                            angle_i = (index_i - middle_index) * scan_msg.angle_increment
                            dist_x = np.cos(angle_i) * range_val
                            dist_y = np.sin(angle_i) * range_val
                            needed_y = np.tan(steering_angle) * dist_x + self.get_parameter('half_vehicle_width').get_parameter_value().double_value
                            if dist_y < needed_y:
                                steering_angle = np.arcsin((dist_y - self.get_parameter('half_vehicle_width').get_parameter_value().double_value) / range_val)
                                print(f'\t Steering angle changed to {steering_angle:.4f} due to obstacle on the LEFT')
                                # if steering_angle < 0:
                                #     break

                # checking for clearance on the left when steering to the left
                elif steering_angle < 0:
                    for i, range_val in enumerate(scan_msg.ranges[start_index:middle_index]):
                        if range_val > 0:
                            index_i = start_index + i
                            angle_i = (middle_index - index_i) * scan_msg.angle_increment
                            dist_x = np.cos(angle_i) * range_val
                            dist_y = np.sin(angle_i) * range_val
                            needed_y = np.tan(steering_angle) * dist_x + self.get_parameter('half_vehicle_width').get_parameter_value().double_value
                            #print(f'\t Angle i: {angle_i}  x: {dist_x}  y: {dist_y}  needed y: {needed_y}')
                            if dist_y < needed_y:
                                steering_angle = -np.arcsin((dist_y - self.get_parameter('half_vehicle_width').get_parameter_value().double_value) / range_val)
                                print(f'\t Steering angle changed to {steering_angle:.4f} due to obstacle on the RIGHT')
                                # if steering_angle > 0:
                                #     break
