...
class FollowTheGapNode(Node):
    def __init__(self):
        ...
        self.safety_margin_gapsize = 1.0 # how much larger gap must be than vehicle width
        self.safety_margin_gapside = 0.5 # how much distance from the edges of the gap shall be ensured, mustn't larger than safety_margin_gapsize/2 otherwise risk of failure
        self.start_index = 0
        self.end_index = 0
    ...
    # Find the start and end indices of the largest sequence of non-zero values in the input vector
        ...
        if current_start is not None and np.mean(input_vector[current_start:]) > max_mean:
            ...
            if gap_space > (self.vehicle_width + self.safety_margin_gapsize):
                ...
                max_start = current_start
        # Eliminate all values that are too cloose to the side of the gap
    ...
    def bubble_algorithm(self, scan_msg, center_idx):
        ...
        while current_index < len(scan_msg.ranges) - 1 and \
            self.scan2scan_distance(center_dist, center_idx, scan_msg.ranges[current_index + 1], current_index + 1, scan_msg.angle_increment) < self.get_parameter('bubble_radius').get_parameter_value().double_value:
            current_index += 1
    ...
    def follow_the_gap_algorithm(self, scan_msg):
        ...
        for elem in diff_indexes:
            self.bubble_algorithm(scan_msg, elem)
        # Create Bubble around closest obstacle
        self.bubble_algorithm(scan_msg, closest_idx)
        # Find best gap
        ...
        try:
            ...
            max_range_index = round((max_gap_end- max_gap_start)/2) + max_gap_start + self.start_index
