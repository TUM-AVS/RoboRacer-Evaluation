...
class RRT(Node):
    def __init__(self):
        ...
        self.waypoints_path = raceline_path #str(self.get_parameter('waypoints_path').value)
        ...
        self.steering_limit = 0.36 # in radians
        self.CELLS_PER_METER = 15
    ...
    def drive_to_target(self, point):
        ...
        # determine velocity
        logit = np.clip(
            (2.0 / (1.0 + np.exp(-5 * (np.abs(np.degrees(angle)) / np.degrees(self.steering_limit))))) - 1.0,
            0.0,
            1.0
        )
...
class PurePursuit:
    ...
    def interpolate_waypoints(self, file_path, segments):
        ...
        points = np.genfromtxt(file_path, delimiter=";")[:-2, 1:3] # Exclude last row, because that closes the loop
